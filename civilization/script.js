...
function setChild(i, j) {
  ...
  positions.push([
    Math.floor((positions[i][0] + positions[j][0]) / 2 + Math.random() * 50),
    Math.floor((positions[i][1] + positions[j][1]) / 2 + Math.random() * 50),
  ]);
  ...
  health.push(100);
  powerAttack.push(
    Math.floor(Math.random() * powerAttack[j] + powerAttack[i]) % 10
  );
  lastAng.push(0);
  rangeVision.push(
    (rangeVision[i] + rangeVision[j]) / 2 + Math.floor(Math.random() * 5 + -5)
  );
  stamina.push(
    Math.floor((stamina[i] + stamina[j]) / 2) +
...
function drawBuilding() {
  for (i = 0; i < house.length; i++) {
    if (house[i][3] == 1) {
      ...
      if (era > 5) img.src = "icons/granja.png";
      // Esperar a que la imagen se cargue antes de dibujarla en el canvas
      // Dibujar la imagen en el canvas en la posición (0, 0)
      ...
      if (Math.floor(Math.random() * 100 < 1))
        food.push([
          house[i][0] + Math.floor(Math.random() * 90 + 10),
          house[i][1] + 10,
          10,
        ]);
      if (Math.floor(Math.random() * 100 < 1))
...
function buildBuilding(i) {
  ...
  setTimeout(() => {
    for (k = 0; k < house.length; k++) {
      if (
        distance(positions[i][0], positions[i][1], house[k][0], house[k][1]) <=
        175
      ) {
        avaliable = false;
      }
  ...
  for (k = 0; k < house.length; k++) {
    ...
    for (j = 0; j < food.length; j++) {
      ...
      if (
        house[k][3] != 0 &&
        food[j][2] > 0 &&
        distance(food[j][0], food[j][1], house[k][0], house[k][1]) < 225
      ) {
  ...
  for (i = 0; i < positions.length; i++) {
    ...
    for (j = 0; j < food.length; j++) {
      if (health[i] > 0 && food[j][2] > 0) {
        if (
          draw &&
          distanceFood(i, j) < rangeVision[i] &&
          calcAng(positions[i][0], positions[i][1], food[j][0], food[j][1]) ==
            lastAng[i]
        ) {
    ...
    for (j = 0; i != j && j < positions.length; j++) {
      if (
        health[j] > 0 &&
        health[i] > 0 &&
        distancePeople(i, j) < (rangeVision[i] * 3) / 4
      ) {
        var num = Math.floor(Math.random() * 100);
        if (
          genes[j] != lord &&
          (num > coop[i] || genHated[i].charAt(6) == genes[j].charAt(6))
        ) {
          //eliminar
          ...
          genes[j] == lord ||
          num <= coop[i] ||
          genes[i].charAt(1) == genes[j].charAt(1) ||
          genes[i].charAt(2) == genes[j].charAt(2)
        ) {
          //cooperar
...
function drawPositions(i, ang) {
  ...
  //for player
  if (players != 0) {
    ctx.fillText("Yo:" + age[0], 20, 170 - 145);
    ctx.fillText("St:" + Math.trunc(stamina[0]), 20, 170 + 5);
    ctx.fillText("Ht:" + health[0], 20, 170 - 10);
    ctx.fillText("Fo:" + supply[0], 20, 170 - 25);
    ctx.fillText("Ge:" + genes[0], 20, 170 - 40);
    ctx.fillText("MT:" + maxTemp[0], 10 + 10, 170 - 55);
    ctx.fillText("mT:" + minTemp[0], 10 + 10, 170 - 70);
    ctx.fillText("At:" + powerAttack[0], 10 + 10, 170 - 85);
    ctx.fillText("Co:" + coop[0], 10 + 10, 170 - 100);
    ctx.fillText("Rv:" + rangeVision[0], 10 + 10, 170 - 115);
    ctx.fillText("Gh:" + genHated[0], 10 + 10, 170 - 130);
  }
  ...
  for (i = 0; i < positions.length; i++) {
    ...
    if (health[i] > 0) {
      ...
      if (stay > 1) {
        ...
        //movement for player
        if (i < players) {
          ang = calcAng(positions[i][0], positions[i][1], mouseX, mouseY);
        }
...
function ActionPlayer(event) {
  ...
  var canvas = document.getElementById("canvas");
  var rect = canvas.getBoundingClientRect(); // Obtener el tamaño y posición del canvas

  // Calcular la escala del canvas en relación con el tamaño visual y el tamaño real
  var scaleX = canvas.width / rect.width; // Relación horizontal
  var scaleY = canvas.height / rect.height; // Relación vertical

  // Ajustar las coordenadas del mouse en función de la escala
...
function drawWar() {
  for (i = players; i < positions.length; i++) {
    if (health[i] > 0 && (genes[i] == inwar[0] || genes[i] == inwar[1])) {
      ...
      ctx.drawImage(
        img,
        positions[i][0] + Math.cos(lastAng[i] - Math.PI / 6) * 10,
        positions[i][1] + Math.sin(lastAng[i] - Math.PI / 6) * 10,
        25,
        25
      );
    }
  ...
  for (i = 0; niebla && i < positions.length; i++) {
    if (health[i] > 0) {
      for (j = 0; j < neb.length; j++) {
        if (
          distance(positions[i][0], positions[i][1], neb[j][0], neb[j][1]) <=
          neb[j][2]
        ) {
          rangeVision[i] = Math.floor(
            Math.random() * (aux[i] - (neb[j][2] % 10))
          );
        }
        if (Math.floor(Math.random() * 2) == 0) neb[j][2]++;
...
function spendStamina() {
  for (i = 0; i < stamina.length; i++) {
    ...
    if (1 == Math.floor(Math.random() * 20)) {
      ...
      stamina[i] += desc;
      supply[i] -= desc;
      if (stamina[i] > 100) stamina[i] = 100;
    }
  }
}
...
function earthquake() {
  ...
  if (Math.floor(Math.random() * 300) != 1) {
    for (i = 0; i < quakes.length; i++) {
      ...
      ctx.beginPath();
      ctx.arc(
        Math.floor(quakes[i][0]),
        Math.floor(quakes[i][1]),
        quakes[i][2],
        0,
        Math.PI * 2
      );
...
function wind() {
  ...
  for (i = 0; i < windwaves.length; i++) {
    ...
    ctx.beginPath();
    ctx.arc(
      Math.floor(windwaves[i][0]),
      Math.floor(windwaves[i][1]),
      windwaves[i][2],
      0,
      Math.PI * 2
    );
    ...
    for (j = 0; j < food.length; j++) {
      if (
        food[j][2] > 0 &&
        distance(food[j][0], food[j][1], windwaves[i][0], windwaves[i][1]) <
          windwaves[i][2] + 1 &&
        distance(food[j][0], food[j][1], windwaves[i][0], windwaves[i][1]) >
          windwaves[i][2] - 1
      ) {
...
function storm() {
  ...
  for (i = 0; i < storms.length; i++) {
    ...
    for (j = 0; j < positions.length; j++) {
      if (
        health[j] > 0 &&
        distance(positions[j][0], positions[j][1], storms[i][0], storms[i][1]) <
          storms[i][2] + 10 &&
        distance(positions[j][0], positions[j][1], storms[i][0], storms[i][1]) >
          storms[i][2] - 10
      ) {
    ...
    for (j = 0; j < food.length; j++) {
      if (
        food[j][2] > 0 &&
        distance(food[j][0], food[j][1], storms[i][0], storms[i][1]) <
          storms[i][2] + 1 &&
        distance(food[j][0], food[j][1], storms[i][0], storms[i][1]) >
          storms[i][2] - 1
      ) {
...
var txz = [];
function toxicZone() {
  if (era > 10 && Math.floor(Math.random() * 300) == 1) {
  ...
  for (i = 0; i < txz.length; i++) {
    ...
    for (j = 0; j < positions.length; j++) {
      if (
        health[j] > 0 &&
        distance(positions[j][0], positions[j][1], txz[i][0], txz[i][1]) <
          txz[i][2]
      ) {
        health[j] -= 1;
      }
    ...
    for (j = 0; j < food.length; j++) {
      if (
        food[j][2] > 0 &&
        distance(food[j][0], food[j][1], txz[i][0], txz[i][1]) < txz[i][2]
      ) {
        food[j][2]--;
      }
    }
...
function comprobationPandemia() {
  ...
  if (contlives < 100 || Math.floor(Math.random() * 100) > 98) return;
...
function pandemia() {
  ...
  for (i = 0; i < positions.length; i++) {
    if (health[i] > 0 && sick.charAt(1) == genes[i].charAt(1)) {
      ...
      for (j = 0; j < positions.length; j++) {
        if (j != i && distancePeople(i, j) < 150) health[j] -= 0.5;
      }
    }
  }
}
...
function war(i) {
  ...
  if (genes[i] == inwar[0]) buscar = inwar[1];
  else buscar = inwar[0];
  ...
  for (i = 0; i < trotamundos.length; i++) {
    ...
    for (j = 0; j < positions.length; j++) {
      ...
      if (
        ...
        if (trotamundos[i][2] < 15) trotamundos[i][2] += 0.05;
      }
    }
...
window.onload = () => {
  intro = setInterval(() => {
    ...
    ctx.shadowColor = "cyan"; // Color de la sombra
    ctx.shadowBlur = 50; // Tamaño del difuminado de la sombra
    ctx.fillText("CIVILIZATION", canvas.width / 2 - 360, canvas.height / 2);
    ctx.shadowBlur = 0; // Tamaño del difuminado de la sombra
    ctx.globalAlpha = 1; // fade rate
  }, 10);
};
